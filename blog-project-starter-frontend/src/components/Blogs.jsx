...
function Blogs() {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged(function(user){
            if(user){
              ...
              if(user.uid==="rXWPg67qOwZVdF254Ji4udIQYz32"){
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`http://localhost:4000/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("http://localhost:4000/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
